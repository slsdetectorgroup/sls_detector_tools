...
def load_file(fname, n_frames, skipframes = 0):
    f = open(fname, 'rb')
    f.seek( skipframes * (header_size+frame_size) )
    image = np.zeros((nrows,ncols,n_frames), dtype = np.uint16)
    for i in range(n_frames):
        f.read(header_size) #frame header TODO! inspect
        image[:,:,i] = np.fromfile(f, dtype = np.uint16, count = nrows*ncols).reshape((nrows,ncols))
        
    return image

def load_file_with_calibration(fname, n_frames, pedestal, calibration,
                            skipframes=100):
    t0 = time.time()
    print(f'Processing: {fname}')
    image = np.zeros((nrows, ncols))
    
    calibrated_images = np.zeros((n_frames,nrows, ncols))
