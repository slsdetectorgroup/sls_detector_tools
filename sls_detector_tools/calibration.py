...
xrf = {'In':3.29,'Sc': 4.1, 'Ti': 4.5, 'Cr': 5.4, 'Fe': 6.4, 'Cu': 8.00, 'Ge': 9.9, 'Zr': 15.8,
...
def do_vrf_scan(detector, xraybox, pixelmask = None, 
    ...
    t = (1000*cfg.calibration.vrf_scan_exptime)/min([data[:,:,np.argmin(np.abs(x-vrf[i]))].mean() for i in range(detector.nmod)])
